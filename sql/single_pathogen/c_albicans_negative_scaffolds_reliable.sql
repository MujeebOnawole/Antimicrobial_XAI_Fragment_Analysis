-- C. albicans Specific Negative Scaffolds Analysis (Using Reliable Compounds - Complete Version)

WITH c_albicans_fragments AS (
    -- Get all negative scaffolds for C. albicans (from reliable compounds only)
...
c_albicans_specific_negative_scaffolds AS (
    -- C. albicans specific negative scaffolds (not in other pathogens)
    SELECT fragment_id
...
scaffold_stats AS (
    ...
    FROM c_albicans_specific_negative_scaffolds cans
...
active_with_negative_attribution AS (
    ...
    WHERE rc.has_mismatch = 0  -- Only reliable compounds
      ...
      AND cf.attribution_score < 0  -- With negative scaffold attribution
),
...
consistency_check AS (
    ...
    JOIN reliable_compounds rc ON cf.compound_id = rc.compound_id AND cf.pathogen_id = rc.pathogen_id
    WHERE cf.pathogen_id = 'c_albicans'
      AND cf.fragment_id IN (SELECT fragment_id FROM c_albicans_specific_negative_scaffolds)
      AND rc.has_mismatch = 0  -- Only reliable compounds
      AND rc.reliability_level = 'RELIABLE'
    GROUP BY cf.fragment_id
)

-- Final results: Complete C. albicans-specific negative scaffolds with all examples
SELECT 
    als.rank,
    als.fragment_id,
    als.fragment_smiles,
    ...
    -- Active with negative attribution (expected case for negative scaffolds)
