-- C. albicans Specific Positive Substituents Analysis (Using Reliable Compounds - Complete Version)

WITH c_albicans_fragments AS (
    -- Get all positive substituents for C. albicans (from reliable compounds only)
...
c_albicans_specific_positive_substituents AS (
    -- C. albicans specific positive substituents (not in other pathogens)
    SELECT fragment_id
...
substituent_stats AS (
    ...
    FROM c_albicans_specific_positive_substituents caps
...
consistency_check AS (
    ...
    JOIN reliable_compounds rc ON cf.compound_id = rc.compound_id AND cf.pathogen_id = rc.pathogen_id
    WHERE cf.pathogen_id = 'c_albicans'
      AND cf.fragment_id IN (SELECT fragment_id FROM c_albicans_specific_positive_substituents)
      AND rc.has_mismatch = 0  -- Only reliable compounds
      AND rc.reliability_level = 'RELIABLE'
    GROUP BY cf.fragment_id
)

-- Final results: Complete C. albicans-specific positive substituents with all examples
SELECT 
    als.rank,
    als.fragment_id,
    als.fragment_smiles,
