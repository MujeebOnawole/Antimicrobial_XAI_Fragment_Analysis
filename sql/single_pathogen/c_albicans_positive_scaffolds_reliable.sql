-- C. albicans Specific Positive Scaffolds Analysis (Using Reliable Compounds - Complete Version)

WITH c_albicans_fragments AS (
    -- Get all positive scaffolds for C. albicans (from reliable compounds only)
...
c_albicans_specific_positive_scaffolds AS (
    -- C. albicans specific positive scaffolds (not in other pathogens)
    SELECT fragment_id
...
scaffold_stats AS (
    ...
    FROM c_albicans_specific_positive_scaffolds caps
    JOIN fragments f ON caps.fragment_id = f.fragment_id
    JOIN compound_fragments cf ON caps.fragment_id = cf.fragment_id AND cf.pathogen_id = 'c_albicans'
    JOIN reliable_compounds rc ON cf.compound_id = rc.compound_id AND cf.pathogen_id = rc.pathogen_id
    WHERE rc.has_mismatch = 0  -- Ensure all compounds are reliable
      AND rc.reliability_level = 'RELIABLE'
    GROUP BY caps.fragment_id, f.smiles
...
-- Inactive examples with positive scaffold attribution (TN with positive attribution)
inactive_with_positive_attribution AS (
    ...
    WHERE rc.has_mismatch = 0  -- Only reliable compounds
      ...
      AND cf.attribution_score > 0  -- But positive scaffold attribution
),

-- Active examples with negative scaffold attribution (TP with negative attribution)
active_with_negative_attribution AS (
    ...
    WHERE rc.has_mismatch = 0  -- Only reliable compounds
      ...
      AND cf.attribution_score < 0  -- But negative scaffold attribution
),
...
consistency_check AS (
    ...
    JOIN reliable_compounds rc ON cf.compound_id = rc.compound_id AND cf.pathogen_id = rc.pathogen_id
    WHERE cf.pathogen_id = 'c_albicans'
      AND cf.fragment_id IN (SELECT fragment_id FROM c_albicans_specific_positive_scaffolds)
      AND rc.has_mismatch = 0  -- Only reliable compounds
      AND rc.reliability_level = 'RELIABLE'
    GROUP BY cf.fragment_id
)

-- Final results: Complete C. albicans-specific positive scaffolds with all examples
SELECT 
    als.rank,
    als.fragment_id,
    als.fragment_smiles,
