-- S. aureus Specific Positive Scaffolds Analysis (Using Reliable Compounds - Complete Version)
-- This query includes all the original columns plus examples

WITH s_aureus_fragments AS (
    -- Get all positive scaffolds for S. aureus (from reliable compounds only)
...
s_aureus_specific_positive_scaffolds AS (
    -- S. aureus specific positive scaffolds (not in other pathogens)
    SELECT fragment_id
...
scaffold_stats AS (
    ...
    FROM s_aureus_specific_positive_scaffolds saps
...
consistency_check AS (
    ...
    JOIN reliable_compounds rc ON cf.compound_id = rc.compound_id AND cf.pathogen_id = rc.pathogen_id
    WHERE cf.pathogen_id = 's_aureus'
      AND cf.fragment_id IN (SELECT fragment_id FROM s_aureus_specific_positive_scaffolds)
      AND rc.has_mismatch = 0  -- Only reliable compounds
      AND rc.reliability_level = 'RELIABLE'
    GROUP BY cf.fragment_id
)

-- Final results: Complete S. aureus-specific positive scaffolds with all examples
SELECT 
    als.rank,
    als.fragment_id,
    als.fragment_smiles,
