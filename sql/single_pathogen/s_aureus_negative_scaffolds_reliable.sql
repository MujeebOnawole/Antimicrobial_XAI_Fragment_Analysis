-- S. aureus Specific Negative Scaffolds Analysis (Using Reliable Compounds - Complete Version)

WITH s_aureus_fragments AS (
    -- Get all negative scaffolds for S. aureus (from reliable compounds only)
...
s_aureus_specific_negative_scaffolds AS (
    -- S. aureus specific negative scaffolds (not in other pathogens)
    SELECT fragment_id
...
scaffold_stats AS (
    ...
    FROM s_aureus_specific_negative_scaffolds sans
...
consistency_check AS (
    ...
    JOIN reliable_compounds rc ON cf.compound_id = rc.compound_id AND cf.pathogen_id = rc.pathogen_id
    WHERE cf.pathogen_id = 's_aureus'
      AND cf.fragment_id IN (SELECT fragment_id FROM s_aureus_specific_negative_scaffolds)
      AND rc.has_mismatch = 0  -- Only reliable compounds
      AND rc.reliability_level = 'RELIABLE'
    GROUP BY cf.fragment_id
)

-- Final results: Complete S. aureus-specific negative scaffolds with all examples
SELECT 
    als.rank,
    als.fragment_id,
    als.fragment_smiles,
