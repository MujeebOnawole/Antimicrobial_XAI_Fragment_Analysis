-- E. coli Specific Negative Scaffolds Analysis (Using Reliable Compounds - Complete Version)
...
lowest_attribution_examples AS (
    ...
    JOIN compound_fragments cf ON als.fragment_id = cf.fragment_id AND cf.pathogen_id = 'e_coli'
    JOIN reliable_compounds rc ON cf.compound_id = rc.compound_id AND cf.pathogen_id = rc.pathogen_id
...
consistency_check AS (
    ...
    JOIN reliable_compounds rc ON cf.compound_id = rc.compound_id AND cf.pathogen_id = rc.pathogen_id
    WHERE cf.pathogen_id = 'e_coli'
      AND cf.fragment_id IN (SELECT fragment_id FROM e_coli_specific_negative_scaffolds)
      AND rc.has_mismatch = 0  -- Only reliable compounds
      AND rc.reliability_level = 'RELIABLE'
    GROUP BY cf.fragment_id
)

-- Final results: Complete E. coli-specific negative scaffolds with all examples
SELECT 
    als.rank,
    als.fragment_id,
    als.fragment_smiles,
