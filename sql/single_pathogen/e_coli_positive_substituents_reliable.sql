-- E. coli Specific Positive Substituents Analysis (Using Reliable Compounds - Complete Version)

WITH e_coli_fragments AS (
    -- Get all positive substituents for E. coli (from reliable compounds only)
...
e_coli_specific_positive_substituents AS (
    -- E. coli specific positive substituents (not in other pathogens)
    SELECT fragment_id
...
substituent_stats AS (
    ...
    FROM e_coli_specific_positive_substituents ecps
...
consistency_check AS (
    ...
    JOIN reliable_compounds rc ON cf.compound_id = rc.compound_id AND cf.pathogen_id = rc.pathogen_id
    WHERE cf.pathogen_id = 'e_coli'
      AND cf.fragment_id IN (SELECT fragment_id FROM e_coli_specific_positive_substituents)
      AND rc.has_mismatch = 0  -- Only reliable compounds
      AND rc.reliability_level = 'RELIABLE'
    GROUP BY cf.fragment_id
)

-- Final results: Complete E. coli-specific positive substituents with all examples
SELECT 
    als.rank,
    als.fragment_id,
    als.fragment_smiles,
