-- S. aureus Specific Negative Substituents Analysis (Using Reliable Compounds - Complete Version)
...
consistency_check AS (
    ...
    JOIN reliable_compounds rc ON cf.compound_id = rc.compound_id AND cf.pathogen_id = rc.pathogen_id
    WHERE cf.pathogen_id = 's_aureus'
      AND cf.fragment_id IN (SELECT fragment_id FROM s_aureus_specific_negative_substituents)
      AND rc.has_mismatch = 0  -- Only reliable compounds
      AND rc.reliability_level = 'RELIABLE'
    GROUP BY cf.fragment_id
)

-- Final results: Complete S. aureus-specific negative substituents
SELECT 
    als.rank,
    als.fragment_id,
    als.fragment_smiles,
