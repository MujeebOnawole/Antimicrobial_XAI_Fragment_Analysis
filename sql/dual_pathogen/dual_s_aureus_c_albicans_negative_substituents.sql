-- Dual Pathogen Negative Substituents Analysis: S. aureus + C. albicans (Using Reliable Compounds)
-- Substituents present in S. aureus AND C. albicans but NOT in E. coli

WITH target_pathogen_negative_substituents AS (
    -- Get substituents that appear with negative attribution in S. aureus AND C. albicans
...
e_coli_substituents AS (
    -- Get substituents present in E. coli
...
dual_specific_negative_substituents AS (
    -- S. aureus + C. albicans specific negative substituents (not in E. coli)
    SELECT fragment_id
...
substituent_stats AS (
    -- Calculate statistics for each substituent across target pathogens
    SELECT 
        dsns.fragment_id,
...
aggregated_substituent_stats AS (
    -- Aggregate statistics across target pathogens for each substituent
    SELECT 
        fragment_id,
        fragment_smiles,
        ...
        ROW_NUMBER() OVER (ORDER BY SUM(tn_count) DESC, AVG(avg_attribution) ASC) as rank
...
pathogen_examples AS (
    SELECT DISTINCT
        ...
        cp.molecular_weight,
        cp.logp,
        ROW_NUMBER() OVER (PARTITION BY ass.fragment_id, cf.pathogen_id ORDER BY cf.attribution_score ASC) as rank
...
consistency_check AS (
    ...
    FROM compound_fragments cf
    JOIN reliable_compounds rc ON cf.compound_id = rc.compound_id AND cf.pathogen_id = rc.pathogen_id
    WHERE cf.fragment_id IN (SELECT fragment_id FROM dual_specific_negative_substituents)
...
-- Final results: Complete S. aureus + C. albicans dual negative substituents
SELECT 
    ass.rank,
    ass.fragment_id,
    ass.fragment_smiles,
    ...
    ROUND(ass.overall_min_attribution::numeric, 4) as overall_min_attribution,
    ROUND(ass.avg_inactivity_rate_percent::numeric, 2) as avg_inactivity_rate_percent,
    
    -- Per-pathogen breakdown
    STRING_AGG(
        ss.pathogen_id || ': ' || ss.total_compounds || ' compounds (' || 
        ss.tp_count || ' TP, ' || ss.tn_count || ' TN) | Avg Attr: ' || 
        ROUND(ss.avg_attribution::numeric, 3) || ' | Inactivity: ' || 
        ss.inactivity_rate_percent || '%',
        ' || '
    ...
    STRING_AGG(
        cc.pathogen_id || ': ' || ROUND((cc.negative_appearances::numeric / cc.total_appearances::numeric * 100), 1) || 
        '% negative (' || cc.negative_appearances || '/' || cc.total_appearances || ') | Range: ' ||
...
GROUP BY ass.rank, ass.fragment_id, ass.fragment_smiles, ass.pathogen_count,
         ...
         ass.overall_min_attribution, ass.avg_inactivity_rate_percent
ORDER BY ass.rank;