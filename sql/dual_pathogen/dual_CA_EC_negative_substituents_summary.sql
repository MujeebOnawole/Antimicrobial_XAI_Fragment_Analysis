-- ====================================================================================
-- QUERY 3: Dual C. albicans + E. coli Negative Substituents Summary
-- ====================================================================================
...
dual_specific_negative_substituents AS (
    ...
    WHERE fragment_id NOT IN (SELECT fragment_id FROM s_aureus_substituents)
),
...
top_substituents AS (
    SELECT 
        COUNT(CASE WHEN total_tn_count >= 20 THEN 1 END) as substituents_with_20plus_tn,
        COUNT(CASE WHEN total_tn_count >= 40 THEN 1 END) as substituents_with_40plus_tn,
        COUNT(CASE WHEN total_tn_count >= 100 THEN 1 END) as substituents_with_100plus_tn,
...
SELECT 
    'Dual C.albicans + E.coli Negative Substituents Summary (Reliable Compounds Only)' as analysis_type,
...
GROUP BY os.total_dual_substituents, os.avg_tn_per_substituent, os.avg_tp_per_substituent,
         ...
         ts.substituents_70plus_inactivity_rate, ts.substituents_high_negative_attribution; WITHIN GROUP (ORDER BY total_tn_count) as tn_count_q75,
