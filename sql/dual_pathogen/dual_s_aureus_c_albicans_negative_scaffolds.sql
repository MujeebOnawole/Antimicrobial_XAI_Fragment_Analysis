-- Dual Pathogen Negative Scaffolds Analysis: S. aureus + C. albicans (Using Reliable Compounds)
-- Scaffolds present in S. aureus AND C. albicans but NOT in E. coli

WITH target_pathogen_negative_scaffolds AS (
    -- Get scaffolds that appear with negative attribution in S. aureus AND C. albicans
...
dual_specific_negative_scaffolds AS (
    -- S. aureus + C. albicans specific negative scaffolds (not in E. coli)
    SELECT fragment_id
...
consistency_check AS (
    ...
    FROM compound_fragments cf
    JOIN reliable_compounds rc ON cf.compound_id = rc.compound_id AND cf.pathogen_id = rc.pathogen_id
    WHERE cf.fragment_id IN (SELECT fragment_id FROM dual_specific_negative_scaffolds)
...
-- Final results: Complete S. aureus + C. albicans dual negative scaffolds
SELECT 
    ass.rank,
    ass.fragment_id,
    ass.fragment_smiles,
