-- Dual Pathogen Positive Substituents Analysis: S. aureus + C. albicans (Using Reliable Compounds)
-- Substituents present in S. aureus AND C. albicans but NOT in E. coli

WITH target_pathogen_positive_substituents AS (
    -- Get substituents that appear with positive attribution in S. aureus AND C. albicans
...
dual_specific_positive_substituents AS (
    -- S. aureus + C. albicans specific positive substituents (not in E. coli)
    SELECT fragment_id
...
consistency_check AS (
    ...
    FROM compound_fragments cf
    JOIN reliable_compounds rc ON cf.compound_id = rc.compound_id AND cf.pathogen_id = rc.pathogen_id
    WHERE cf.fragment_id IN (SELECT fragment_id FROM dual_specific_positive_substituents)
...
-- Final results: Complete S. aureus + C. albicans dual positive substituents
SELECT 
    ass.rank,
    ass.fragment_id,
    ass.fragment_smiles,
