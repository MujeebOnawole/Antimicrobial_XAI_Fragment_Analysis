...
-- SUMMARY 1: Dual S. aureus + C. albicans Negative Scaffolds Summary
-- ====================================================================================

WITH target_pathogen_negative_scaffolds AS (
...
dual_specific_negative_scaffolds AS (
    SELECT fragment_id
    FROM target_pathogen_negative_scaffolds
...
scaffold_stats AS (
    ...
    FROM dual_specific_negative_scaffolds dsns
...
top_scaffolds AS (
    SELECT 
        COUNT(CASE WHEN total_tn_count >= 20 THEN 1 END) as scaffolds_with_20plus_tn,   -- 10+ per pathogen
        COUNT(CASE WHEN total_tn_count >= 40 THEN 1 END) as scaffolds_with_40plus_tn,   -- 20+ per pathogen
        COUNT(CASE WHEN total_tn_count >= 100 THEN 1 END) as scaffolds_with_100plus_tn, -- 50+ per pathogen
...
SELECT 
    'Dual S.aureus + C.albicans Negative Scaffolds Summary (Reliable Compounds Only)' as analysis_type,
    ...
    ts.scaffolds_with_40plus_tn,
    ts.scaffolds_with_100plus_tn,
    ts.scaffolds_50plus_inactivity_rate,
    ts.scaffolds_70plus_inactivity_rate,
    ts.scaffolds_high_negative_attribution,
    
    -- Quality percentages
    ROUND((ts.scaffolds_with_20plus_tn::numeric / os.total_dual_scaffolds * 100), 1) as pct_scaffolds_20plus_tn,
    ROUND((ts.scaffolds_50plus_inactivity_rate::numeric / os.total_dual_scaffolds * 100), 1) as pct_scaffolds_50plus_inactivity,
    ROUND((ts.scaffolds_high_negative_attribution::numeric / os.total_dual_scaffolds * 100), 1) as pct_scaffolds_high_negative_attribution,
...
GROUP BY os.total_dual_scaffolds, os.avg_tn_per_scaffold, os.avg_tp_per_scaffold,
         ...
         ds.attribution_q75, ts.scaffolds_with_20plus_tn, ts.scaffolds_with_40plus_tn,
         ts.scaffolds_with_100plus_tn, ts.scaffolds_50plus_inactivity_rate,
         ts.scaffolds_70plus_inactivity_rate, ts.scaffolds_high_negative_attribution;

-- ====================================================================================
-- INSTRUCTIONS FOR OTHER 3 SUMMARY QUERIES:
-- ====================================================================================

/*
SUMMARY 2: Dual S. aureus + C. albicans Positive Scaffolds Summary
- Change attribution_score <= -0.1 to >= 0.1 throughout
- Change 'Negative Scaffolds' to 'Positive Scaffolds' in analysis_type
- Change COUNT(CASE WHEN rc.target = 0 THEN 1 END) >= 3 to COUNT(CASE WHEN rc.target = 1 THEN 1 END) >= 3
- Change MIN(cf.attribution_score) to MAX(cf.attribution_score)
- Change inactivity_rate_percent to activity_rate_percent
- Change tn_count references to tp_count in relevant sections
- Change scaffolds_high_negative_attribution to scaffolds_high_attribution (>= 0.2)

SUMMARY 3: Dual S. aureus + C. albicans Negative Substituents Summary
- Change f.fragment_type = 'scaffold' to 'substituent' throughout
- Change 'Scaffolds' to 'Substituents' in analysis_type and variable names
- Keep all negative attribution logic

SUMMARY 4: Dual S. aureus + C. albicans Positive Substituents Summary
- Change f.fragment_type = 'scaffold' to 'substituent' throughout
- Change 'Scaffolds' to 'Substituents' in analysis_type and variable names
- Apply all positive attribution changes from Summary 2
*/