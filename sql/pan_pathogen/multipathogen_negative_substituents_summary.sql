-- Summary Statistics for Multipathogen Negative Substituents (Using Reliable Compounds)
-- Substituents present in ALL THREE pathogens with negative attribution

WITH all_pathogen_negative_substituents AS (
    -- Get substituents that appear with negative attribution in ALL three pathogens
...
substituent_stats AS (
    ...
    FROM all_pathogen_negative_substituents apns
...
top_substituents AS (
    SELECT 
        COUNT(CASE WHEN total_tn_count >= 30 THEN 1 END) as substituents_with_30plus_tn,  -- 10+ per pathogen
        COUNT(CASE WHEN total_tn_count >= 60 THEN 1 END) as substituents_with_60plus_tn,  -- 20+ per pathogen
        COUNT(CASE WHEN total_tn_count >= 150 THEN 1 END) as substituents_with_150plus_tn, -- 50+ per pathogen
...
SELECT 
    'Multipathogen Negative Substituents Summary (Reliable Compounds Only)' as analysis_type,
    ...
    -- Quality metrics
    ts.substituents_with_30plus_tn,
    ts.substituents_with_60plus_tn,
    ts.substituents_with_150plus_tn,
    ts.substituents_50plus_inactivity_rate,
    ts.substituents_70plus_inactivity_rate,
    ts.substituents_high_negative_attribution,
    
    -- Quality percentages
    ROUND((ts.substituents_with_30plus_tn::numeric / os.total_multipathogen_substituents * 100), 1) as pct_substituents_30plus_tn,
    ROUND((ts.substituents_50plus_inactivity_rate::numeric / os.total_multipathogen_substituents * 100), 1) as pct_substituents_50plus_inactivity,
    ROUND((ts.substituents_high_negative_attribution::numeric / os.total_multipathogen_substituents * 100), 1) as pct_substituents_high_negative_attribution,
...
GROUP BY os.total_multipathogen_substituents, os.avg_tn_per_substituent, os.avg_tp_per_substituent,
         ...
         ds.attribution_q75, ts.substituents_with_30plus_tn, ts.substituents_with_60plus_tn,
         ts.substituents_with_150plus_tn, ts.substituents_50plus_inactivity_rate,
