-- Multipathogen Negative Substituents Analysis (Using Reliable Compounds - Complete Version)
...
consistency_check AS (
    ...
    FROM compound_fragments cf
    JOIN reliable_compounds rc ON cf.compound_id = rc.compound_id AND cf.pathogen_id = rc.pathogen_id
    WHERE cf.fragment_id IN (SELECT fragment_id FROM all_pathogen_negative_substituents)
      AND rc.has_mismatch = 0  -- Only reliable compounds
      AND rc.reliability_level = 'RELIABLE'
    GROUP BY cf.fragment_id, cf.pathogen_id
)

-- Final results: Complete multipathogen negative substituents
SELECT 
    ass.rank,
    ass.fragment_id,
    ass.fragment_smiles,
