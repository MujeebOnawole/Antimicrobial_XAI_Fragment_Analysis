...
Uses representative ChEMBL compound IDs and fixes white space in center.

Exemplars (with representative ChEMBL compounds containing these fragments):
- SA: Fragment 925 -> CHEMBL3623427
- EC: Fragment 206183 -> CHEMBL4162805
- CA: Fragment 227610 -> CHEMBL3261662
"""
...
# VERIFIED EXEMPLARS with representative ChEMBL compounds
compounds = {
    'SA_CHEMBL3623427': {
        'smiles': 'C1=CC=C(OCC2=CCC=CN2)C=C1',
        'fragment_id': 925,
        'chembl_id': 'CHEMBL3623427',
        'rank': 150,
...
def create_radar_only(compound_id, compound_data, select_data, output_path):
    """Create radar plot with all 8 properties - no white space in center."""
    ...
    # FIXED: Fill from 0 to lower bound first (lighter shade) to eliminate white space
    ax.fill_between(angles, 0, select_lower, alpha=0.15, color=compound_data['zone_color'], zorder=0)

    # Then fill the main SELECT range band
    ax.fill_between(angles, select_lower, select_upper, alpha=0.35, color=compound_data['zone_color'],
    ...
    chembl = compound_data['chembl_id']
    ax.plot(angles, compound_norm, 'o-', linewidth=2.5, markersize=8,
            color=compound_data['color'], label=chembl, zorder=3)
    ...
    # Title with ChEMBL ID
    fig.text(0.5, 0.92, f'Physicochemical Profile: {chembl}',
...
def create_combined_plot(compound_id, compound_data, select_data, props, output_path):
    ...
    ax1.text(0.5, -0.08, f"Exemplar: {chembl}", transform=ax1.transAxes,
             ha='center', fontsize=12, style='italic', fontfamily='Arial')

    # Radar panel
    ax2 = fig.add_subplot(122, projection='polar')

    # Fill from 0 to lower (lighter) to eliminate white space
    ax2.fill_between(angles, 0, select_lower, alpha=0.15, color=compound_data['zone_color'], zorder=0)

    # Main SELECT range band
    ax2.fill_between(angles, select_lower, select_upper, alpha=0.35, color=compound_data['zone_color'],
                     label=f"{compound_data['select_rule']} range", zorder=1)
    ...
    fig.suptitle(f"{compound_data['name'].title()}: {chembl}",
...
print("With ChEMBL compound IDs and no white space in center")
print("=" * 70)

for compound_id, data in compounds.items():
    chembl = data['chembl_id']
    print(f"\n{data['name']}:")
    print(f"  ChEMBL ID: {chembl}")
    ...
    # Radar only
    radar_file = OUTPUT_DIR / f'{chembl}_radar_8props.png'
    ...
    # Combined
    combined_file = OUTPUT_DIR / f'{chembl}_combined_8props.png'
    create_combined_plot(compound_id, data, select_ranges, props, combined_file)
    print(f"  [OK] {combined_file.name}")

print("\n" + "=" * 70)
print("COMPLETE!")
print("Files use ChEMBL IDs and have no white space in center.")
print("=" * 70)
